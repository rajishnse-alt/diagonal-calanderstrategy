...
if True:  # always render — individual cells show "—" if data missing
    ...
    st.markdown(
        ...
        f"{_fmt_s(_spcl_ce_h)} | {_fmt_s(_ce_spcl)}"
        f"<br><span style='color:var(--muted);font-size:9px;font-weight:400;'>L: {_fmt_s(_spcl_ce_l)}</span></td>"
        ...
        f"padding:4px 8px;'>"
        f"<span style='color:var(--muted);font-size:9px;font-weight:400;'>{_5m_spcl_atm} </span>"
        f"{_fmt_s(_spcl_pe_h)} | {_fmt_s(_pe_spcl)}"
        f"<br><span style='color:var(--muted);font-size:9px;font-weight:400;'>L: {_fmt_s(_spcl_pe_l)}</span></td>"
